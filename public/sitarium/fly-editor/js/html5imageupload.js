...
(function (window, $, undefined) {
	...
	$.html5imageupload = function html5imageupload(options, element) {
		...
		//buttons
		this.button				= {}
		this.button.edit		= '<div class="btn btn-xs btn-info btn-edit" title="' + (this.options.editTitle || 'Edit') + '"><i class="glyphicon glyphicon-pencil"></i></div>'
		...
		//
		this.button.undo		= '	<button type="button" class="fly-editor_undo btn btn-warning btn-xs"">' +
								  '		<span class="glyphicon glyphicon-share-alt"></span>' +
								  '	</button>';
		//
		this.button.zoomin		= '<div class="btn btn-xs btn-default btn-zoom-in" title="' + (this.options.zoominTitle || 'Zoom in') + '"><i class="glyphicon glyphicon-resize-full"></i></div>';
		this.button.zoomout		= '<div class="btn btn-xs btn-default btn-zoom-out" title="' + (this.options.zoomoutTitle || 'Zoom out') + '"><i class="glyphicon glyphicon-resize-small"></i></div>';
		this.button.zoomreset	= '<div class="btn btn-xs btn-default btn-zoom-reset" title="' + (this.options.zoomresetTitle || 'Fullsize') + '"><i class="glyphicon glyphicon-fullscreen"></i></div>';
//		this.button.rotatecw	= '<div class="btn btn-xs btn-default btn-rotate-cw" title="' + (this.options.cwTitle || 'Rotate clockwise') + '"><i class="glyphicon glyphicon-share"></i></div>';
//		this.button.rotateccw	= '<div class="btn btn-xs btn-default btn-rotate-ccw" title="' + (this.options.ccwTitle || 'Rotate counter clockwise') + '"><i class="glyphicon glyphicon-share icon-flipped"></i></div>';
		this.button.cancel		= '<div class="btn btn-xs btn-danger btn-cancel" title="' + (this.options.cancelTitle || 'Cancel') + '"><i class="glyphicon glyphicon-remove"></i></div>';
		this.button.done		= '<div class="btn btn-xs btn-success btn-ok" title="' + (this.options.okTitle || 'Ok') + '"><i class="glyphicon glyphicon-ok"></i></div>';
		this.button.del			= '<div class="btn btn-xs btn-danger btn-del" title="' + (this.options.delTitle || 'Delete') + '"><i class="glyphicon glyphicon-trash"></i></div>';
...
//			}
		...
		handleFile: function(event, element) {
		   ...
		   // _self.reset();
		    ...
		    if (_self.options.onAfterSelectImage) _self.options.onAfterSelectImage.call(_self, _self);
		},
			...
			$.ajax({
				...
				success: function(response) {
					...
					if (response.status == "success") {
						...
						$(element).data('final_url', response.url);
						if (options.canvas != true) {
		...
		imageFinal: function() {
			...
			//undo option
			if (options.buttonUndo != false && _self.options.onUndo) {
				$(tools).append($(_self.button.undo).click(function(e){
					_self.options.onUndo.call(_self,_self);
				}));
			}
...
/*			if (options.buttonRotateccw != false) {
		...
		undo: function() {
			var _self				= this;
			if (_self.options.onUndo) _self.options.onUndo.call(_self,_self);
		}
