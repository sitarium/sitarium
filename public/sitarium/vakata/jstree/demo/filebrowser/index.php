    ...
    public function lst($id, $with_root = false)
    {
        ...
        foreach ($lst as $item) {
            ...
            if (is_dir($dir.DIRECTORY_SEPARATOR.$item)) {
                $res[] = ['text' => $item, 'children' => true,  'id' => $this->id($dir.DIRECTORY_SEPARATOR.$item), 'icon' => 'folder'];
            } else {
                $res[] = ['text' => $item, 'children' => false, 'id' => $this->id($dir.DIRECTORY_SEPARATOR.$item), 'type' => 'file', 'icon' => 'file file-'.substr($item, strrpos($item, '.') + 1)];
            }
        }
        if ($with_root && $this->id($dir) === '/') {
            $res = [['text' => basename($this->base), 'children' => $res, 'id' => '/', 'icon' => 'folder', 'state' => ['opened' => true, 'disabled' => true]]];
        }
