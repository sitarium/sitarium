...
namespace vakata\database
{
    ...
    class Query
    {
        ...
        public function execute($vars = [])
        {
            ...
            $this->num = (is_object($this->rsl) || is_resource($this->rsl)) && is_callable([$this->drv, 'nf']) ? (int) @$this->drv->nf($this->rsl) : 0;
    ...
    class DBC implements IDB
    {
        ...
        public function get($sql, $data = [], $key = null, $skip_key = false, $mode = 'assoc')
        {
            return $this->query($sql, $data)->result($key, $skip_key, $mode);
        }

        public function all($sql, $data = [], $key = null, $skip_key = false, $mode = 'assoc')
        {
            return $this->get($sql, $data, $key, $skip_key, $mode)->get();
        }

        public function one($sql, $data = [], $mode = 'assoc')
        {
        ...
        {
            if ($this->que && is_callable([$this->que, $method])) {
                try {
                    return call_user_func_array([$this->que, $method], $args);
                } catch (Exception $e) {
    ...
    class DBCCached extends DBC
    {
        ...
        public function cache($expires, $sql, $data = [], $key = null, $skip_key = false, $mode = 'assoc')
        {
            $arg = func_get_args();
            array_shift($arg);
            $key = md5(serialize($arg));
            if (! $this->cache_inst) {
                return call_user_func_array([$this, 'all'], $arg);
            }

            $tmp = $this->cache_inst->get($key, $this->cache_nmsp);
            if (! $tmp) {
                $this->cache_inst->prep($key, $this->cache_nmsp);
                $tmp = call_user_func_array([$this, 'all'], $arg);
    ...
    class mysqli_driver extends ADriver
    {
        ...
        public function nextr($result)
        {
            if ($this->mnd) {
                ...
                if (! call_user_func_array([$result, 'bind_result'], $tmp)) {
                    return false;
                }
                if (! $result->fetch()) {
                    return false;
        ...
        public function execute($sql, $data = [])
        {
            ...
            if ($sql->param_count) {
                ...
                call_user_func_array([$sql, 'bind_param'], $ref);
            }
            $rtrn = $sql->execute();
            if (! $this->mnd) {
                $sql->store_result();
    ...
    class postgre_driver extends ADriver
    {
        ...
        public function execute($sql, $data = [])
        {
            ...
            $temp = (is_array($data) && count($data)) ? pg_query_params($this->lnk, $sql, $data) : pg_query_params($this->lnk, $sql, []);
            if (! $temp) {
